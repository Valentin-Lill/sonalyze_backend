...
from app.analysis.audio_generator import MeasurementSignalConfig, get_signal_timing, generate_measurement_signal
...
def _handle_analysis_run(
    ...
    if source == "impulse_response":
        ...
        # Use recording and regenerate the sweep signal for deconvolution
        # This is useful when the exact sweep used during measurement is known
        # (generated by the backend) and doesn't need to be uploaded
        rec_name = data.get("recording_upload", "recording")
        rec_path = uploads_dir / rec_name
        ...
        # Regenerate the measurement signal with matching sample rate
        config = MeasurementSignalConfig(sample_rate=fs_r)
        full_signal, _ = generate_measurement_signal(config)
        
        # Extract just the sweep portion for deconvolution
        timing = get_signal_timing(config)
        sweep_start_sample = int(timing["sweep_start"] * fs_r)
        sweep_end_sample = int(timing["sweep_end"] * fs_r)
        sweep = full_signal[sweep_start_sample:sweep_end_sample]
        ...
        logger.info(
            f"Deconvolved using regenerated sweep (fs={fs_r}, "
            f"sweep_len={len(sweep)}, rec_len={len(rec)})"
        )
